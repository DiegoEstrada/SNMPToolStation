...
class ConfigurationAdmin(object):
    ...
    def __init__(self,ip,getway,interface=None):
        print("CONSTRUCTOR")
        self.ip = ip
        self.getway = getway
        #self.routeNetwork = routerNetwork
        if interface is None:
            self.interface = generateTap()
            if( addRule() == 0):
                a = 0
                #verifyConnection()
        else:
    ...
    def generateTap(self):
        ...
        if( retval == 0):
            ...
            #print(tapVariable[0])
            comand = "ifconfig " + tapVariable[0]  + " " +self.ip + " up"
    ...
    def addRule(self, netRoute=None):
        """Agrega reglas para permitir la comunicacion con otros routers a traves de la interfaz vietual"""
        rnD = ""
        if netRoute is None:
            rnD = self.routeNetwork
        else:
            rnD = netRoute
        
        rule = subprocess.run("route add -net  " + rnD +" gw "+self.getway+ " dev "+ self.interface,stdout=subprocess.PIPE,  stderr=subprocess.STDOUT, shell=True)
    ...
    def downloadConfigFile(self,ip=None):
        ...
        rutaArchivo = '/home/diego/Desktop/CONFIGROUTER/' +hostToConnect+ '.txt' # Ruta del archivo a subir
        ...
        ftp.quit()
        localfile.close()
        rule = subprocess.run("chmod 777  "+hostToConnect+".txt",stdout=subprocess.PIPE,  stderr=subprocess.STDOUT, shell=True)
    ...
    def querySNMPInfo(self, ip=None):
        ...
        if ip is None:
            ...
            hostToConnect = ip
        dictio.update({'ip':hostToConnect})
